...
How did I know the `base.url`? I read the [documentation](https://developer.nytimes.com/docs/articlesearch-product/1/routes/articlesearch.json/get). Notice that this `base.url` also includes the *response format*(`.jston`), so we do not need to configure that directly.
