...
How did I know the `base.url`? I read the [documentation](https://developer.nytimes.com/docs/articlesearch-product/1/routes/articlesearch.json/get). Notice that this `base.url` also includes the *response format*(`.json`), so we do not need to configure that directly.
...
That is great. But we only have 10 items. The original response said we had 48 hits! Which means we have to make 48/10, or 5 requests to get them all. Sounds like a job for iteration!

First, let's write a function that passes a search term and a page number, and returns a dataframe of articles:

```{r error = F, message = F}
nytapi <- function(term = NULL, page){
    base.url = "http://api.nytimes.com/svc/search/v2/articlesearch.json"
    ...
    print(paste("Scraping page: ", as.character(page)))
    
    return(response_df$response$docs)
}

docs <- nytapi("'John Mearsheimer'", page = 2)
```
...
docs_df <- map_dfr((1:pages), ~nytapi_slow(term = search_term, page = .))
```
...
by_month <- docs_df %>% 
  dplyr::count(floor_date(date, "month")) %>%
