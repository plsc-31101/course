...
\NormalTok{* }\StringTok{  Bulleted list item 1}

\NormalTok{* }\StringTok{  Item 2}

\BaseNTok{    * Item 2a}

\BaseNTok{    * Item 2b}

\NormalTok{1. }\StringTok{ Numbered list item 1}

\NormalTok{1. }\StringTok{ Item 2. The numbers are incremented automatically in the output.}
...
\addcontentsline{toc}{subsubsection}{\texttt{NaN} means ``not a number.'' It is an undefined value.}
...
\subsection{\texorpdfstring{Use \texttt{arrange()} to Row-order Data in a Principled Way}{Use arrange() to Row-order Data in a Principled Way}}\label{use-arrange-to-row-order-data-in-a-principled-way}}
...
Oh, you would like to sort on life expectancy in \textbf{desc}ending order? Then use \texttt{desc()}.
...
That allowed us to calculate the mean \texttt{gdpPercap} for each continent.
...
That is already quite powerful, but it gets even better! You are not limited to defining only one new variable in \texttt{summarize()}.
...
For this, we can use the \texttt{mutate()} function, which is similar to \texttt{summarize()} except that it creates new variables in the same data frame that you pass into it.
...
We can also use \texttt{mutate()} to create new variables prior to (or even after) summarizing the information.
...
You will notice that, in our long dataset, \texttt{obstype\_year} actually contains two pieces of information, the observation type (\texttt{pop}, \texttt{lifeExp}, or \texttt{gdpPercap}) and the \texttt{year}.
...
\NormalTok{lifeExp_}\DecValTok{1967}\NormalTok{ <-}\StringTok{ }\NormalTok{gap }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{filter}\NormalTok{(year}\OperatorTok{==}\DecValTok{1967}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{pull}\NormalTok{(lifeExp)}

\NormalTok{lifeExp_}\DecValTok{1977}\NormalTok{ <-}\StringTok{ }\NormalTok{gap }\OperatorTok{%>%}\StringTok{ }
...
\addcontentsline{toc}{subsubsection}{Categorical variables (Factors)}
...
\begin{center}\includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-248-1} \includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-248-2} \includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-248-3} \includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-248-4} \end{center}
...
\CommentTok{#> % Date and time: Tue, Oct 20, 2020 - 15:46:34}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{mod <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(lifeExp }\OperatorTok{~}\StringTok{ }\NormalTok{country }\OperatorTok{+}\StringTok{ }\NormalTok{year }\OperatorTok{+}\StringTok{ }\KeywordTok{log}\NormalTok{(pop) }\OperatorTok{+}\StringTok{ }\KeywordTok{log}\NormalTok{(gdpPercap), }\DataTypeTok{data =}\NormalTok{ gap)}

\NormalTok{mod_df <-}\StringTok{ }\KeywordTok{tidy}\NormalTok{(mod) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{slice}\NormalTok{(}\DecValTok{2}\OperatorTok{:}\DecValTok{142}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{separate}\NormalTok{(term, }\DataTypeTok{into =} \KeywordTok{c}\NormalTok{(}\StringTok{"term"}\NormalTok{, }\StringTok{"country"}\NormalTok{), }\DataTypeTok{sep =} \StringTok{"country"}\NormalTok{) }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{select}\NormalTok{(country, estimate, std.error)}
  
\NormalTok{mod_df <-}\StringTok{ }\NormalTok{mod_df }\OperatorTok{%>%}
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{low =}\NormalTok{ estimate }\OperatorTok{-}\StringTok{ }\KeywordTok{qnorm}\NormalTok{((}\DecValTok{1}\FloatTok{-0.95}\NormalTok{)}\OperatorTok{/}\DecValTok{2}\NormalTok{)}\OperatorTok{*}\NormalTok{(std.error),}
         \DataTypeTok{hi =}\NormalTok{ estimate }\OperatorTok{+}\StringTok{ }\KeywordTok{qnorm}\NormalTok{((}\DecValTok{1}\FloatTok{-0.95}\NormalTok{)}\OperatorTok{/}\DecValTok{2}\NormalTok{)}\OperatorTok{*}\NormalTok{(std.error))}

\KeywordTok{ggplot}\NormalTok{(mod_df, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}  \KeywordTok{fct_reorder}\NormalTok{(country, estimate),}
                   \DataTypeTok{y =}\NormalTok{ estimate, }
                   \DataTypeTok{ymin =}\NormalTok{ low, }
                   \DataTypeTok{ymax =}\NormalTok{ hi)) }\OperatorTok{+}
\StringTok{  }\KeywordTok{geom_pointrange}\NormalTok{(}\DataTypeTok{colour =} \StringTok{"red"}\NormalTok{, }\DataTypeTok{size =} \FloatTok{.2}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{ylab}\NormalTok{(}\StringTok{"Marginal Effect on Life Expectancy"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{xlab}\NormalTok{(}\StringTok{"Country"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{coord_flip}\NormalTok{() }\OperatorTok{+}
\StringTok{  }\KeywordTok{theme}\NormalTok{(}\DataTypeTok{axis.text.y =} \KeywordTok{element_text}\NormalTok{(}\DataTypeTok{size =} \DecValTok{5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Subsetting a list works in the same way as subsetting an atomic vector. However, there is one important difference: \texttt{{[}} will always return a list. \texttt{{[}{[}} and \texttt{\$}, as described below, let you pull out the components of the list.

The \href{https://r4ds.had.co.nz/vectors.html\#lists-of-condiments}{``pepper shaker photos''} in R for Data Science are a splendid visual explanation of the different ways to get stuff out of a list. Highly recommended.
...
\NormalTok{my_list <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DataTypeTok{a =} \DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{, }
                ...
                \DataTypeTok{d =} \KeywordTok{list}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{, }\DecValTok{-5}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
Typically, however, your strings will be one column of a data frame, and you will want to use \texttt{filter} instead:
...
Like the other \texttt{stringr} functions that return a list, you can use \texttt{simplify\ =\ TRUE} to return a matrix:
...
This did not work correctly. Because \texttt{if()} can only handle a single \texttt{TRUE}/\texttt{FALSE} value, it only checked the first row of the data frame. That row contained \texttt{28.801}, so it generated a vector of length 1704 with each value being \texttt{0}.
...
Within \texttt{response\_df\$response}, we can extract a number of interesting results, including the number of total hits, as well as information on the first 10 documents:
...
\CommentTok{#> [11] "section_name"            "_id"                    }
\CommentTok{#> [13] "word_count"              "uri"                    }
\CommentTok{#> [15] "type_of_material"        "print_section"          }
...
Now, we are ready to iterate over each page. First, let's review what we have done so far:
...
\begin{center}\includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-443-1} \end{center}
...
\addcontentsline{toc}{subsubsection}{Challenge 1: Find the CSS Selector.}
...
\hypertarget{challenge-2-make-a-function.}{%
\subsubsection*{Challenge 2: Make a Function.}\label{challenge-2-make-a-function.}}
...
Compare the first 5 rows and 5 columns of the \texttt{dtm} and \texttt{dtm.weighted}. What do you notice?
...
\begin{center}\includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-474-1} \end{center}
...
\begin{center}\includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-476-1} \end{center}
...
STM has its own unique preprocessing functions and procedure, which I have coded below. Notice that we are going to use the \texttt{TEXT.NO.NOUN} column, which contains all the text of the articles without proper nouns (which I removed earlier).
...
\hypertarget{challenge-1.-18}{%
\subsubsection*{Challenge 1.}\label{challenge-1.-18}}
\addcontentsline{toc}{subsubsection}{Challenge 1.}
...
\hypertarget{challenge-2.-17}{%
\subsubsection*{Challenge 2.}\label{challenge-2.-17}}
\addcontentsline{toc}{subsubsection}{Challenge 2.}
...
\begin{center}\includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-506-1} \includegraphics[width=0.7\linewidth]{plsc31101_files/figure-latex/unnamed-chunk-506-2} \end{center}
