...
### Detect matches with `str_detect()`
...
To extract the actual text of a match, use `str_extract()`. To show that off, we are going to need a more complicated example. I am going to use the [Harvard sentences](https://en.wikipedia.org/wiki/Harvard_sentences). These are provided in `stringr::sentences`:
...
### Split on a match with `str_split()`
...
[R for Data Science](https://r4ds.had.co.nz), licensed under [Creative Commons Attribution-NonCommercial-NoDerivs 3.0](https://creativecommons.org/licenses/by-nc-nd/3.0/us/).
